...
describe("Testing spendings functionality", () => {
  ...
  it("Add spending to the category and get spending info", async () => {
    ...
    await request(url).post("/api/spendings/add_spending_to_category").expect(200)
                      .send(
                        {spending_id: spending_1.spending_id, category_id: category_1.category_id})
                      .expect({});
    ...
    await Helpers.DeleteGroupCategoryAndSpending(group_1.group_id, category_1.category_id,
                                                 spending_1.spending_id);
  });
...
describe("Testing spendings expected errors", () => {
  ...
  it("Not enough rights for accessing spendings", async () => {
    const spending_1 = {
      description: "spending",
      value: 13,
      currency: "USD",
    ...
    const right_spending_id = 1;
    const wrong_spending_id = 2;
    const right_category_id = 1
    const wrong_category_id = 2;
    await request(url).post("/api/spendings/update_spending").expect(ErrorCode.Forbidden)
                      .send({
                              spending_id: wrong_spending_id,
                              description: spending_1.description,
                              value: spending_1.value,
                              currency: spending_1.currency,
                              date: spending_1.date,
    ...
    await request(url).post("/api/spendings/delete_spending").expect(ErrorCode.Forbidden)
                      .send({spending_id: wrong_spending_id})
                      .expect((res) => {
                        expect(res.body.hasOwnProperty("error")).toBeTruthy();
                      });
    await request(url).post("/api/spendings/get_spending_info").expect(ErrorCode.Forbidden)
                      .send({spending_id: wrong_spending_id})
                      .expect((res) => {
                        expect(res.body.hasOwnProperty("error")).toBeTruthy();
                      });
    // Wrong spending
    await request(url).post("/api/spendings/add_spending_to_category").expect(ErrorCode.Forbidden)
                      .send({spending_id: wrong_spending_id, category_id: right_category_id})
                      .expect((res) => {
                        expect(res.body.hasOwnProperty("error")).toBeTruthy();
                      });
    // Wrong category
    await request(url).post("/api/spendings/add_spending_to_category").expect(ErrorCode.Forbidden)
                      .send({spending_id: right_spending_id, category_id: wrong_category_id})
                      .expect((res) => {
                        expect(res.body.hasOwnProperty("error")).toBeTruthy();
                      });
  });
